...
namespace LibreLancer.Net
{
    ...
    {
        [JsonIgnore] public string Url { get; set; } = null!;
        [JsonPropertyName("application")] public required string Application { get; set; }
        [JsonPropertyName("registerEnabled")] public bool RegisterEnabled { get; set; }
        [JsonPropertyName("loginDifficulty")] public int LoginDifficulty { get; set; }
    ...
    public static class HttpAuthentication
    {
        ...
        public static async Task<string?> Login(this HttpClient client, AuthInfo info, string username, string password)
        {
            try
            {
                ...
                {
                    FLLog.Error("Http",
                        $"Login failed for {username} {result.StatusCode}. Response:\n {await result.Content.ReadAsStringAsync()}");
                    return null;
                }
